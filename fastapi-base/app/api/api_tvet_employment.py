"""
TVET Employment API - Technical & Vocational Education Training employment stats
"""
...
from app.models.model_indicator_details import TVETEmploymentDetail

router = APIRouter(prefix="/api/tvet-employment", tags=["tvet_employment_detail"])


class TVETEmploymentCreate(BaseModel):
    province: Optional[str] = None
    year: Optional[int] = None
    quarter: Optional[int] = None
    month: Optional[int] = None
    total_graduates: Optional[int] = None
    employed_graduates: Optional[int] = None
    employment_rate: Optional[float] = None
    skilled_employment_rate: Optional[float] = None
    average_salary: Optional[float] = None
    certification_rate: Optional[float] = None
...
class TVETEmploymentResponse(BaseModel):
    id: int
    province: Optional[str]
    year: Optional[int]
    total_graduates: Optional[int]
    employed_graduates: Optional[int]
...
class TVETEmploymentListResponse(BaseModel):
    total: int
    page: int
    page_size: int
    total_pages: int
    data: list[TVETEmploymentResponse]


@router.get("", response_model=TVETEmploymentListResponse)
def list_tvet_employment(
    year: Optional[int] = None,
    province: Optional[str] = None,
    skip: int = 0,
    limit: int = 50,
    ...
    """List TVET employment data"""
    query = db.query(TVETEmploymentDetail)
    
    if year:
        query = query.filter(TVETEmploymentDetail.year == year)
    if province:
        query = query.filter(TVETEmploymentDetail.province == province)
    
    total = query.count()
    items = query.order_by(desc(TVETEmploymentDetail.year)).offset(skip).limit(limit).all()
...
@router.get("/by-id/{id}", response_model=TVETEmploymentResponse)
def get_tvet_employment(id: int, db: Session = Depends(get_db)):
    """Get TVET employment by ID"""
    record = db.query(TVETEmploymentDetail).filter(TVETEmploymentDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
    return record


@router.post("", response_model=TVETEmploymentResponse)
def create_or_update_tvet_employment(data: TVETEmploymentCreate, db: Session = Depends(get_db)):
    """Create or update TVET employment data"""
    query = db.query(TVETEmploymentDetail).filter(
        TVETEmploymentDetail.province == data.province,
        TVETEmploymentDetail.year == data.year
    )
    ...
    new_record = TVETEmploymentDetail(**data.model_dump())
    db.add(new_record)
    db.commit()
...
def delete_tvet_employment(id: int, db: Session = Depends(get_db)):
    """Delete TVET employment record"""
    record = db.query(TVETEmploymentDetail).filter(TVETEmploymentDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
...
def extract_from_database_sources(
    ...
    """
    Extract TVET Employment từ database sources sử dụng LLM
    ...
    from call_llm.extract_tvet_employment import extract_tvet_employment
        ...
        for post in posts:
            try:
                ...
                if data:
                    ...
                    insert_query = text("""
                        INSERT INTO tvet_employment_detail 
                        (province, year, quarter, month, total_graduates, employed_graduates,
                         employment_rate, skilled_employment_rate, average_salary, certification_rate,
                         training_completion_rate, data_status, data_source, document_type)
                        VALUES (:province, :year, :quarter, :month, :total_graduates, :employed_graduates,
                                :employment_rate, :skilled_employment_rate, :average_salary, :certification_rate,
                                :training_completion_rate, :data_status, :data_source, :document_type)
                        ON CONFLICT (province, year) DO UPDATE SET
                            total_graduates = EXCLUDED.total_graduates,
                            employed_graduates = EXCLUDED.employed_graduates,
