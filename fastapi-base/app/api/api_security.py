"""
Security Detail API - CRUD endpoints for security/public safety statistics
"""
...
from app.models.model_indicator_details import SecurityDetail

router = APIRouter(prefix="/api/security", tags=["security_detail"])
...
class SecurityCreate(BaseModel):
    ...
    crime_cases: Optional[int] = None
    solved_cases: Optional[int] = None
    traffic_accidents: Optional[int] = None
    fire_incidents: Optional[int] = None
    public_safety_score: Optional[float] = None
...
class SecurityResponse(BaseModel):
    ...
    year: Optional[int]
    crime_cases: Optional[int]
    solved_cases: Optional[int]
    traffic_accidents: Optional[int]
    public_safety_score: Optional[float]
...
def list_security(
    ...
    """List security statistics"""
    query = db.query(SecurityDetail)
    
    if year:
        query = query.filter(SecurityDetail.year == year)
    if province:
        query = query.filter(SecurityDetail.province == province)
    
    total = query.count()
    items = query.order_by(desc(SecurityDetail.year)).offset(skip).limit(limit).all()
...
@router.get("/by-id/{id}", response_model=SecurityResponse)
def get_security(id: int, db: Session = Depends(get_db)):
    """Get security statistics by ID"""
    record = db.query(SecurityDetail).filter(SecurityDetail.id == id).first()
    if not record:
...
def create_or_update_security(data: SecurityCreate, db: Session = Depends(get_db)):
    """Create or update security data"""
    query = db.query(SecurityDetail).filter(
        SecurityDetail.province == data.province,
        SecurityDetail.year == data.year
    )
    ...
    new_record = SecurityDetail(**data.model_dump())
    db.add(new_record)
    db.commit()
...
def extract_from_database_sources(
    ...
    """
    Extract Security statistics từ database sources sử dụng LLM
    ...
    from call_llm.extract_security import extract_security_detail
    ...
    if source in ['important_posts', 'both']:
        ...
        query = db.query(ImportantPost).filter(ImportantPost.type_newspaper == 'security')
            ...
            try:
                ...
                if data:
                    ...
                    insert_query = text("""
                        ...
                        (province, year, quarter, month, crime_cases, solved_cases,
                         traffic_accidents, fire_incidents, public_safety_score,
                         data_status, data_source, document_type)
                        VALUES (:province, :year, :quarter, :month, :crime_cases, :solved_cases,
                                :traffic_accidents, :fire_incidents, :public_safety_score,
                                :data_status, :data_source, :document_type)
                        ON CONFLICT (province, year) DO UPDATE SET
                            crime_cases = EXCLUDED.crime_cases,
                            solved_cases = EXCLUDED.solved_cases,
                            traffic_accidents = EXCLUDED.traffic_accidents,
    ...
    if source in ['articles', 'both']:
        ...
        query = db.query(Article).filter(Article.type_newspaper == 'security')
