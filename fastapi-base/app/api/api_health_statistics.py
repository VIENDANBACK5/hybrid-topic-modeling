"""
Health Statistics API - CRUD endpoints
"""
...
from app.models.model_indicator_details import HealthStatisticsDetail

router = APIRouter(prefix="/api/health-statistics", tags=["health_statistics_detail"])


class HealthStatisticsCreate(BaseModel):
    province: Optional[str] = None
    year: Optional[int] = None
    quarter: Optional[int] = None
    month: Optional[int] = None
    hospital_beds: Optional[int] = None
    doctors_per_10k: Optional[float] = None
    health_insurance_coverage: Optional[float] = None
    maternal_mortality_rate: Optional[float] = None
    infant_mortality_rate: Optional[float] = None
...
class HealthStatisticsResponse(BaseModel):
    ...
    year: Optional[int]
    hospital_beds: Optional[int]
    doctors_per_10k: Optional[float]
    health_insurance_coverage: Optional[float]
...
class HealthStatisticsListResponse(BaseModel):
    total: int
    page: int
    page_size: int
    total_pages: int
    data: list[HealthStatisticsResponse]


@router.get("", response_model=HealthStatisticsListResponse)
def list_health_statistics(
    ...
    query = db.query(HealthStatisticsDetail)
    
    if year:
        query = query.filter(HealthStatisticsDetail.year == year)
    if province:
        query = query.filter(HealthStatisticsDetail.province == province)
    
    total = query.count()
    items = query.order_by(desc(HealthStatisticsDetail.year)).offset(skip).limit(limit).all()
...
@router.get("/by-id/{id}", response_model=HealthStatisticsResponse)
def get_health_statistics(id: int, db: Session = Depends(get_db)):
    """Get health statistics by ID"""
    record = db.query(HealthStatisticsDetail).filter(HealthStatisticsDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
    return record


@router.post("", response_model=HealthStatisticsResponse)
def create_or_update_health_statistics(data: HealthStatisticsCreate, db: Session = Depends(get_db)):
    """Create or update health statistics"""
    query = db.query(HealthStatisticsDetail).filter(
        HealthStatisticsDetail.province == data.province,
        HealthStatisticsDetail.year == data.year
    )
    ...
    new_record = HealthStatisticsDetail(**data.model_dump())
    db.add(new_record)
    db.commit()
...
def delete_health_statistics(id: int, db: Session = Depends(get_db)):
    """Delete health statistics record"""
    record = db.query(HealthStatisticsDetail).filter(HealthStatisticsDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
...
def extract_from_database_sources(
    ...
    """
    Extract Health Statistics từ database sources sử dụng LLM
    ...
    from call_llm.extract_medical import extract_health_statistics
    ...
    if source in ['important_posts', 'both']:
        ...
        query = db.query(ImportantPost).filter(ImportantPost.type_newspaper == 'health')
        ...
        for post in posts:
            try:
                ...
                data = extract_health_statistics(
                ...
                if data:
                    ...
                    insert_query = text("""
                        ...
                        (province, year, quarter, month, hospital_beds, doctors_per_10k,
                         health_insurance_coverage, maternal_mortality_rate, infant_mortality_rate,
                         vaccination_rate, data_status, data_source, document_type)
                        VALUES (:province, :year, :quarter, :month, :hospital_beds, :doctors_per_10k,
                                :health_insurance_coverage, :maternal_mortality_rate, :infant_mortality_rate,
                                :vaccination_rate, :data_status, :data_source, :document_type)
                        ON CONFLICT (province, year) DO UPDATE SET
                            hospital_beds = EXCLUDED.hospital_beds,
                            doctors_per_10k = EXCLUDED.doctors_per_10k,
    ...
    if source in ['articles', 'both']:
        ...
        query = db.query(Article).filter(Article.type_newspaper == 'health')
