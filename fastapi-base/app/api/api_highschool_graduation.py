"""
Highschool Graduation API - CRUD endpoints
"""
...
from app.models.model_indicator_details import HighschoolGraduationDetail

router = APIRouter(prefix="/api/highschool-graduation", tags=["highschool_graduation_detail"])


class HighschoolGraduationCreate(BaseModel):
    ...
    math_avg_score: Optional[float] = None
    literature_avg_score: Optional[float] = None
    english_avg_score: Optional[float] = None
...
class HighschoolGraduationResponse(BaseModel):
    id: int
    province: Optional[str]
    year: Optional[int]
    graduation_rate: Optional[float]
    total_candidates: Optional[int]
    passed_candidates: Optional[int]
...
class HighschoolGraduationListResponse(BaseModel):
    total: int
    page: int
    page_size: int
    total_pages: int
    data: list[HighschoolGraduationResponse]


@router.get("", response_model=HighschoolGraduationListResponse)
def list_highschool_graduation(
    year: Optional[int] = None,
    province: Optional[str] = None,
    skip: int = 0,
    limit: int = 50,
    ...
    """List highschool graduation data"""
    query = db.query(HighschoolGraduationDetail)
    
    if year:
        query = query.filter(HighschoolGraduationDetail.year == year)
    if province:
        query = query.filter(HighschoolGraduationDetail.province == province)
    
    total = query.count()
    items = query.order_by(desc(HighschoolGraduationDetail.year)).offset(skip).limit(limit).all()
...
@router.get("/by-id/{id}", response_model=HighschoolGraduationResponse)
def get_highschool_graduation(id: int, db: Session = Depends(get_db)):
    """Get highschool graduation by ID"""
    record = db.query(HighschoolGraduationDetail).filter(HighschoolGraduationDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
    return record


@router.post("", response_model=HighschoolGraduationResponse)
def create_or_update_highschool_graduation(data: HighschoolGraduationCreate, db: Session = Depends(get_db)):
    """Create or update highschool graduation data"""
    query = db.query(HighschoolGraduationDetail).filter(
        HighschoolGraduationDetail.province == data.province,
        HighschoolGraduationDetail.year == data.year
    )
    ...
    new_record = HighschoolGraduationDetail(**data.model_dump())
    db.add(new_record)
    db.commit()
...
def delete_highschool_graduation(id: int, db: Session = Depends(get_db)):
    """Delete highschool graduation record"""
    record = db.query(HighschoolGraduationDetail).filter(HighschoolGraduationDetail.id == id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
...
def extract_from_database_sources(
    ...
    """
    Extract Highschool Graduation từ database sources sử dụng LLM
    ...
    from call_llm.extract_highschool_graduation import extract_highschool_graduation
    ...
    if source in ['important_posts', 'both']:
        ...
        query = db.query(ImportantPost).filter(ImportantPost.type_newspaper == 'education')
        ...
        for post in posts:
            try:
                ...
                data = extract_highschool_graduation(
                ...
                if data:
                    ...
                    insert_query = text("""
                        ...
                        (province, year, quarter, month, graduation_rate, total_candidates, 
                         passed_candidates, average_score, math_avg_score, literature_avg_score,
                         english_avg_score, excellent_rate, fail_rate, data_status, data_source, document_type)
                        VALUES (:province, :year, :quarter, :month, :graduation_rate, :total_candidates,
                                :passed_candidates, :average_score, :math_avg_score, :literature_avg_score,
                                :english_avg_score, :excellent_rate, :fail_rate, :data_status, :data_source, :document_type)
    ...
    if source in ['articles', 'both']:
        ...
        query = db.query(Article).filter(Article.type_newspaper == 'education')
